...
import { ResolverUpdated } from "../generated/ETHRegistry/PermissionedRegistry";
import {
  AddrChanged,
  AddressChanged,
  TextChanged,
...
function seedDomain(tokenId: i32, label: string, resolver: string): string {
  handleLabelRegistered(labelRegisteredEvent(tokenId, label, OWNER, EXPIRY, REGISTRAR, ETH_REGISTRY));
  const node = namehashOf(label);
  handleResolverUpdated(resolverUpdatedEvent(tokenId, resolver));
  return node;
}

const NODE = namehashOf("resolvedemo");
const RESOLVER_ID = resolverId(Address.fromString(RESOLVER), NODE);

describe("resolverLive: ResolverUpdated links resolver (v1 NewResolver semantics)", () => {
  ...
  test("zero resolver clears the link", () => {
    clearStore();
    seedDomain(4, "clearres", RESOLVER);
...
describe("resolverLive: record events", () => {
